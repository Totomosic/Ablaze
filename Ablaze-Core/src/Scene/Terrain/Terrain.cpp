...
namespace Ablaze
{
	...
	Terrain::Terrain(const String& name, const maths::vec2& size, int vertexResolution) : Model(name, new VertexBuffer(vertexResolution * vertexResolution * sizeof(Vertex), BufferLayout::Vertex()), new IndexBuffer((int)((vertexResolution - 1) * (vertexResolution - 1) * 6 * sizeof(GLuint))), maths::vec3(size.x, 0, size.y))
	{
