...
namespace Ablaze
{
	...
	Sphere::Sphere(const String& name, float radius, const Color& color) : Model(name, new VBO((4 * (180 / 10 + 1) * ((360 - 10) / 10 + 1)) * sizeof(Vertex), 0), new IndexBuffer((4 * (180 / 10 + 1) * ((360 - 10) / 10 + 1)) * sizeof(GLuint)))
	{
		this->radius = radius;
		Create(color);
	}
