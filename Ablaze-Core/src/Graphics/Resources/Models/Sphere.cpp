...
namespace Ablaze
{
	...
	Sphere::Sphere(const String& name, float radius, const Color& color) : Model(name, new VertexBuffer((4 * (180 / 10 + 1) * ((360 - 10) / 10 + 1)) * sizeof(Vertex), BufferLayout::Vertex()), new IndexBuffer((4 * (180 / 10 + 1) * ((360 - 10) / 10 + 1)) * sizeof(GLuint)), maths::vec3(radius * 2))
	{
		this->radius = radius;
		Create(color);
	}
